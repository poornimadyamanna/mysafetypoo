...
import { PredefinedMessageService } from "../../services/admin/predefinedmsg.service";
...
export class PredefinedMessageController {
    ...
    createPredefinedMessage = async (req: AuthenticatedRequest, res: Response) => {
        try {
            ...
            const { title, message, moduleType, audioId } = req.body;

            const createdMessage = await this.predefinedMessageService.createPredefinedMessage({ title, message, moduleType, userId, audioId });
